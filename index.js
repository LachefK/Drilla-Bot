...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
      		...
      		mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD PRIVATE BOT*',
					...
					ownerG: '[❗] Karib? Este é um recurso especial para o Karib❌',
					ownerB: '[❗] Karib? Este é um recurso especial para o Karib ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Karib. Registre-se para fazer amizade com o Drilla Bot por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar seunome|17\n\n──「 DRILLA BOT 」──`,
			...
			const ownerNumber = ["5521969395519@s.whatsapp.net"] // coloque seu numero
			const mod = [ownerNumber,"5521969395519@s.whatsapp.net"]//seu numero
			const adminbotnumber = ["5521969395519@s.whatsapp.net"]//seu numero
			const frendsowner = ["5521969395519@s.whatsapp.net"]//seu numero
			const premium = ["5521969395519@s.whatsapp.net","557196445183@s.whatsapp.net",] 
		...
		if (isGroupAdmins) return reply('Seu racista de merda eu até te fumaria, mas você é ADM. Tenta a sorte aí membro comum')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Seu racista de merda ${sender.split("@")[0]} você será fumado em 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("No meu Bloco, não permitimos racistas, que isso sirva de exemplo 🚶")
		}, 0)
	}
		...
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('Seu merda eu até te fumaria, mas você é ADM. Tenta a sorte aí membro comum')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Link de Telegram é minha piroca ${sender.split("@")[0]} você será fumado`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Peça permissão da próxima vez")
		}, 0)
	}
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Link de WhatsApp é minha piroca ${sender.split("@")[0]} você será fumado`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Tchau Spammer ${sender.split("@")[0]} você será fumado`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Tchau Spammer de merda")
		}, 0)
	}
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Tchau tiktoker ${sender.split("@")[0]} você será fumado`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Porra de tiktok filho")
		}, 0)
	}
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Manda link na buceta da mãe ${sender.split("@")[0]} você será fumad`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Smoking that pussy boy")
		}, 0)
	}
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Kkkkkkkwai mano vai se fude ${sender.split("@")[0]} você será fumado`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Tchau nerd frustrado")
		}, 0)
	}
	...
	if (messagesC.includes("mt1")){
		if (!isPremium) return reply(mess.only.premium)
			...
			if (messagesC.includes("von")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/von.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	...
	if (messagesC.includes(".faustinho")){
			...
			switch(command) {
				...
				case 'addfoto':
					...
					break
				case 'bloqueados':
					teks = 'Esta é a lista de números fumados :\n'
				...
				case 'addvip':  
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('A marca o alvo que você quer chutar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '╭────「 *PREMIUM👑* 」──*\n│+ *Número* : \n│+ *Expirado*: *30 Days*\n│+ *Status*: *ATIVO*\n│ Valeu por atualizar para premium\n*╰──────「 *posição* 」────'
...
break
case 'desligar':
				if (!isOwner) return reply('Comando so para o meu dono, quem é você?')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 para ativar, 0 para desativar, lerdão você em KKKKK')
					}
					break
				case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
...
case 'casal':
      ...
      if (!isGroup) return reply(mess.only.group)
                        ...
                        teks = `
                  Paradas!🤚🤚\n\n1=🤚🤭@${o1.jid.split('@')[0]}🤚🤭\n\n\n2=🤚🤭@${o2.jid.split('@')[0]}🤚🤭\n\n\n3=🤚🤭@${o3.jid.split('@')[0]}🤚🤭\n\n\n4=🤚🤭@${o4.jid.split('@')[0]}🤚🤭\n\n\n5=🤚🤭@${o5.jid.split('@')[0]}🤚🤭\n\n\nMultadass por serem gostosas demais😈`
                        member.push(o1.jid)
                        member.push(o2.jid)
                        member.push(o3.jid)
                        member.push(o4.jid)
...
wew = fs.readFileSync('./assets/pp.png')
                client.sendMessage(from, wew, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "𝐃𝐑𝐈𝐋𝐋𝐀 𝐁𝐎𝐓", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('kk/sticker/images.webp')} } }, caption: help(prefix) })
				  break
				case 'help1':
				case 'menu1':
...
case 'inthecrib':
                ...
                break
					break
case 'vipmenu':
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Karib ou digite *#daftarvip* para comprar acesso Premium!' ,text, { quoted: mek })
		...
		break
               ...
               if (!isPremium) return reply(mess.only.premium)
                   ...
                   case 'pack':
					...
					case 'lista':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('Você é o Karib??')
...
case 'antilink':
				...
				//INFO BOT
case 'owner':
                    client.sendMessage(from, {displayname: "KARIB", vcard: vcard}, MessageType.contact, { quoted: mek})
                    client.sendMessage(from, 'Contato do meu dono ai, se floodar vai ser fumado aviso',MessageType.text, { quoted: mek} )
                    break
case 'speed':
                case 'ping':
...
case 'daftarvip': 
				...
				case 'boanoite':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'boa noite rapaziada️'})
					break
case 'boatarde':
					memein = await kagApi.memeindo()
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia rapaziada️'})
					break
case 'belle':                 
                 ...
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*Você comprou a água dela ctz*')
				break
				case 'belle1':
					memein = await kagApi.memeindo()
...
case 'belle2':
					...
					client.sendMessage(from, buffer, sticker, {quoted: mek, caption: 'para de ser punheteiro'})
					break
case 'belle3':
					memein = await kagApi.memeindo()
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'comrpou a água dela ctz.'})
					break
case 'termux':
